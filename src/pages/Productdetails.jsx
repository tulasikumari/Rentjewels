...
const ProductDetails = () => {
  ...
  useEffect(() => {
    getProductDetailsApi(id)
      .then((res) => {
        console.log("Product Details:", res.data); // Log product details
        setProduct(res.data.product);
        setProductquantity(res.data.product.quantity); // Ensure correct quantity is set
      })
      .catch((error) => {
  ...
  const handleRentNow = (e) => {
    ...
    addToCartApi(formData)
      .then((res) => {
        ...
        navigate("/ship"); // Redirect to cart or shipping page
      })
      .catch((err) => {
